...
import org.jjly.framework.encode.EncodeUtils;
...
public class ServletUtils
{
	...
	public static boolean checkIfNoneMatchEtag(HttpServletRequest request, HttpServletResponse response, String etag)
	{
		...
		if (headerValue != null) {
			...
			if (!("*".equals(headerValue))) {
				...
				do { String currentToken = commaTokenizer.nextToken();
			...
			if (conditionSatisfied) {
				label81: response.setStatus(304);
			response.setHeader("ETag", etag);
			return false;
			}
		}
	...
	public static void setFileDownloadHeader(HttpServletRequest request, HttpServletResponse response, String fileName)
	{
		...
		catch (java.io.UnsupportedEncodingException browserType)
		{
		}
	}
			...
			if (("".equals(prefix)) || (paramName.startsWith(prefix))) {
				...
				if ((values != null) && (values.length != 0))
				{
					if (values.length > 1){
						...
						currVal = values[0];
						if(unprefixed.startsWith("EQ_is")){
							if(StringUtils.isNotBlank(currVal)){
								if(currVal.equals("1") || currVal.equals("true")){
							...
							params.put(unprefixed, currVal);	
						}
						
						request.setAttribute(paramName, currVal);
					}
				}
			}
		}
