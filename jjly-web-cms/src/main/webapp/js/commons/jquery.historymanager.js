...
;~(function() {
	var flag = false, isIE = !!window.ActiveXObject
			&& /msie (\d)/i.test(navigator.userAgent) ? RegExp['$1'] : false, $pointer = this;
	...
	this.makeIEHistory = function(url) {
		...
		frameDoc.write([
				...
				"function pageLoaded() {",
				"try {top.window.historyManager.fireOnHashChange(\"" + url
